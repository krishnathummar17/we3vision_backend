...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    // Allow only safe, predefined folders
    const allowed = new Set(['uploads', 'media', 'images']);
    let folder = (req.body?.folder || req.query?.folder || 'uploads')
      .toString()
      .trim()
      .toLowerCase();
    if (!allowed.has(folder)) folder = 'uploads';
    ...
    req.uploadFolder = folder; // e.g., 'uploads' | 'media' | 'images'
    cb(null, dest);
  },
